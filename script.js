...
function injectUnifiedNavigationDocks() {
  ...
  // 1. DESKTOP HEADER (Clean Layout - Zero Redundant Top Profile Icons)
    ...
    desktopHeaderContainer.innerHTML = `
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <a href="index.html" class="flex items-center space-x-3">
          <div class="w-8 h-8 rounded-full border border-black dark:border-white flex items-center justify-center font-bold text-xs">C</div>
        ...
        <nav class="hidden md:flex items-center space-x-8 text-[10px] font-bold tracking-[0.2em] text-gray-400 uppercase">
          <a href="index.html" class="${currentPath.includes('index.html') ? 'text-black dark:text-white' : 'hover:text-black'} transition">New Arrivals</a>
          <a href="products.html" class="${currentPath.includes('products.html') ? 'text-black dark:text-white' : 'hover:text-black'} transition">Catalog</a>
          <a href="orders-tracker.html" class="${currentPath.includes('orders-tracker.html') ? 'text-black dark:text-white' : 'hover:text-black'} transition">Orders</a>
          <a href="profile.html" class="${currentPath.includes('profile.html') ? 'text-black dark:text-white' : 'hover:text-black'} transition">Settings</a>
        </nav>
        <button onclick="toggleGlobalThemeEngine()" class="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-50 dark:bg-zinc-800 text-xs">🌓</button>
      </div>
    `;
  }

  // 2. FIXED MOBILE APP HUD DOCK (High-End Luxury Vector Uniformity)
  ...
  mobileDock.innerHTML = `
    ...
    <a href="cart.html" class="flex flex-col items-center justify-center relative transition ${currentPath.includes('cart.html') ? 'text-black dark:text-white scale-105' : 'text-gray-400 dark:text-zinc-600'}">
      ...
      <span class="absolute -top-1 -right-1.5 bg-black dark:bg-white text-white dark:text-black text-[7px] w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold">${totalQty}</span>
